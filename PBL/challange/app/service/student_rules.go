...
func ValidateCreate(reqq *model.CreateStudentRequest) map[string]string {
	...
	if strings.TrimSpace(reqq.MataKuliah) == "" {
		errs["mata_kuliah"] = "wajib diisi"
	}
...
func ValidateReplace(req *model.ReplaceStudentRequest) map[string]string {
	...
	if strings.TrimSpace(req.MataKuliah) == "" {
		errs["mata_kuliah"] = "wajib diisi pada PUT"
	}
...
func ApplyPatch(s *model.Student, req *model.PatchStudentRequest) map[string]string {
	...
	if req.MataKuliah != nil {
		if strings.TrimSpace(*req.MataKuliah) == "" {
			errs["mata_kuliah"] = "wajib diisi pada PACTH"
		} else {
			s.MataKuliah = *req.MataKuliah
		}
	}
	if req.Grade != nil {
		if *req.Grade < 0 || *req.Grade > 4 {
...
func IsEmptyPatch(req *model.PatchStudentRequest) bool {
	...
	return req.NIM == nil && req.Name == nil && req.Grade == nil &&req.MataKuliah == nil && req.IsActive == nil
}
