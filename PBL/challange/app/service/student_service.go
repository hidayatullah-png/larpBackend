...
func (s *StudentService) GetGradeByNIM(c *fiber.Ctx) error {
	nim :=c.Params("nim")
	...
	student,err :=s.repo.FindByNIM(c.Context(), nim)
	if err != nil{
		if errors.Is(err, repository.ErrNotFound) {
			return helper.Fail(c, fiber.StatusNotFound, "Mahasiswa dengan NIM tersebut tidak ditemukan")
		}
		return helper.Fail(c, fiber.StatusInternalServerError, "Gagal mengambil data dari server")
	}
	return helper.Success(c, fiber.StatusOK, "berhasil mengambil nilai Mahasiswa", student)
}
...
func (s *StudentService) Create(c *fiber.Ctx) error {
	...
	req.MataKuliah = strings.TrimSpace (req.MataKuliah)
