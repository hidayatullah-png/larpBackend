...
func NewPool(ctx context.Context) (*pgxpool.Pool, error) {
	...
	cfg.MaxConns = int32(config.GetEnvInt("DB_MAX_CONNS", 10)) //banyak koneksi yang boleh terbuka ada 10
	cfg.MinConns = 2 //jumlah koneksi yang siap siaga ada 2
	cfg.MaxConnLifetime = time.Hour //setiap koneksi ke database memiliki batas umur maksimal selama 1 jam
	cfg.MaxConnIdleTime = 30 * time.Minute //berapa lama koneksi akan ditutup jika menganggur (pengecualian jika koneksi yang tersisa menyentuh minimal idle)
